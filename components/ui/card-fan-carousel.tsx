...
function getSlotConfig(totalCards: number, slot: number) {
  ...
  // Use exact float center for symmetry with even number of cards
  ...
  return {
    ...
    zIndex: 100 - Math.abs(slot - center) * 10,
  };
}
...
export default function SocialCards({ cards }: SocialCardsProps) {
  ...
  useEffect(() => {
    ...
    // Failsafe to ensure isAnimating is reset
      ...
      visibleEntries.forEach(({ el, slot }) => {
        ...
        if (hoveredSlot !== null) {
          ...
          if (slot === hoveredSlot) {
            targetY -= 1.5 * hM;
            targetScale *= 1.08;
            targetZ = 200; // Pop the hovered card to the absolute front
          } else {
            // Un-paralyze edge cards: Give all neighbors a consistent, strong push
            const pushStrength = 7 * (1 + 0.2 * Math.max(0, 3 - distance));
        ...
        gsap.to(el, {
          ...
          duration: 0.5, delay, ease: "elastic.out(1,.75)", overwrite: true,
        });
    ...
    const enterHandlers = visibleEntries.map(({ el, slot }) => {
      ...
      const leaveHandler = () => {
        ...
        // Extremely tight debounce to allow seamless sliding between overlapping cards
      ...
      return { el, enterHandler, leaveHandler };
    });
    ...
    return () => {
      enterHandlers.forEach(({ el, enterHandler, leaveHandler }) => {
      ...
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem] h-[450px] md:h-[550px]">
          {cards.map((card, index) => {
            const content = (
              <div className="relative w-full h-full rounded-2xl overflow-hidden shadow-[0_0_40px_rgba(0,0,0,0.3)] border border-white/10 bg-gradient-to-br from-[#0a0a0a] via-[#111111] to-[#050505] p-6 md:p-8 flex flex-col group">
                ...
                <h3 className="text-xl md:text-2xl font-bold text-white mb-4 leading-tight group-hover:text-white/90 transition-colors">{card.title}</h3>
                {card.description && (
                   <p className="text-xs md:text-sm text-white/60 line-clamp-4 leading-relaxed flex-grow">{card.description}</p>
                )}
                {card.tech && (
                  <div className="flex flex-wrap gap-2 mt-auto pt-4">
                    {card.tech.slice(0, 4).map((t, i) => (
                       <span key={i} className="px-2.5 py-1 bg-white/5 border border-white/10 rounded-md text-[10px] md:text-xs text-white/80">{t}</span>
                    ))}
                  </div>
                )}
              </div>
            ...
            const cardClasses = "fan-card absolute w-[260px] h-[360px] md:w-[320px] md:h-[440px] rounded-2xl cursor-pointer will-change-transform flex-shrink-0";

            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className={cardClasses}>{content}</a>
            ) : (
              <div key={index} className={cardClasses}>{content}</div>
            );
          })}
        </div>
      </div>
